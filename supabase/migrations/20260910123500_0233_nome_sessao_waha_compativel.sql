-- O WAHA 2026.7.2 limita o nome da sessão a 54 caracteres. A reserva anterior
-- gerava 69 (`org_` + UUID da organização + UUID aleatório), então toda conexão
-- nova parava em HTTP 400 antes de criar qualquer sessão remota.
--
-- Forward-fix de 0230: nomes novos têm 45 caracteres. Um retry também cura a
-- identidade longa deixada em FAILED, mas só quando o recibo prova que o remoto
-- nunca foi criado; uma sessão existente jamais é renomeada às cegas.
